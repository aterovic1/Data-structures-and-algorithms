...
public:
    ...
    void ispis(); 
    std::vector<TipK> od_do(TipK a, TipV b);
    void rek(std::vector<TipK> &v, Cvor* k, TipK a, TipK b);
};

template <typename TipK, typename TipV>
std::vector<TipK> BinStabloMapa<TipK, TipV>::od_do(TipK a, TipV b) {
    std::vector<TipK> rez;
    rek(rez, korijen, a, b);
    return rez;
}
...
template <typename TipK, typename TipV>
void BinStabloMapa<TipK, TipV>::rek(std::vector<TipK> &v, Cvor* k, TipK a, TipK b) {
    if(k == nullptr)
        return;
    rek(v, k -> desni, a, b);
    if(k -> kljuc >= a && k -> kljuc <= b)
        v.push_back(k -> kljuc);
    rek(v, k -> lijevi, a, b);
}
...
template <typename TipK, typename TipV>
void BinStabloMapa<TipK, TipV>::ispis() {
    ispisi(korijen);
    std::cout << std::endl;
}

template <typename TipK, typename TipV>
void BinStabloMapa<TipK, TipV>::ispisi(Cvor* c) {
    if(c == nullptr) {
        return;
    }
    ispisi(c -> lijevi);
    ...
    if(c -> roditelj != nullptr)
        std::cout << "kljuc " << c -> kljuc << ", vrijednost " << c -> vrijednost << ", roditelj je -> " << c -> roditelj -> kljuc << "\n";
    else 
        std::cout << "kljuc " << c -> kljuc << ", vrijednost " << c -> vrijednost << " <-- KORIJEN" << "\n";
